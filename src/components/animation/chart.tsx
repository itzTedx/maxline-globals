...
const GridLayer = ({ color }: GridLayerProps) => {
	return (
		<div
			className="mask-[radial-gradient(ellipse_50%_50%_at_50%_50%,#000_60%,transparent_100%)] pointer-events-none absolute inset-0 z-4 h-full w-full bg-[linear-gradient(to_right,var(--grid-color)_1px,transparent_1px),linear-gradient(to_bottom,var(--grid-color)_1px,transparent_1px)] bg-center bg-size-[20px_20px] bg-transparent opacity-70"
			style={
				{
					"--grid-color": color,
...
const EllipseGradient = ({ color }: EllipseGradientProps) => {
	return (
		<div className="absolute inset-0 z-5 flex h-full w-full items-center justify-center">
			...
			>
				<rect fill="url(#paint)" height="180" width="356" />
				<defs>
					<radialGradient
						cx="0"
						cy="0"
...
const Layer1 = ({ color, secondaryColor }: LayerProps) => {
	return (
		<div className="0.6, 0, 1)] absolute top-0 left-0 z-6 transform transition-transform duration-500 ease-[cubic-bezier(0.6, group-hover/animated-card:translate-x-[-50%]">
			<svg
				className="w-[712px]"
				fill="none"
				viewBox="0 0 712 180"
...
const Layer2 = ({ color }: LayerProps) => {
	return (
		<div className="absolute top-0 -left-px h-full w-[356px]">
			<svg
			...
			>
				...
				<defs>
					<clipPath id="clip0_25_384">
						<rect fill="white" height="180" width="356" />
					</clipPath>
				</defs>
			</svg>
			<div className="0.6, 0, 1)] absolute inset-0 z-3 transform bg-linear-to-r from-0% from-transparent to-15% to-white transition-transform duration-500 ease-[cubic-bezier(0.6, group-hover/animated-card:translate-x-full dark:to-black" />
		</div>
	);
};
...
const Layer4 = () => {
	return (
		<div className="group relative h-full w-[356px]">
			<div className="0.6, 0, 1)] absolute inset-0 z-7 flex max-w-[356px] -translate-y-full items-start justify-start bg-transparent p-4 transition-transform duration-500 ease-[cubic-bezier(0.6, group-hover/animated-card:translate-y-0">
				<div className="0.6, 0, 1)] rounded-md border border-zinc-200 bg-white/25 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 ease-[cubic-bezier(0.6, group-hover/animated-card:opacity-100 dark:border-zinc-800 dark:bg-black/25">
					<p className="mb-1 font-semibold text-black text-xs dark:text-white">
