...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const title =
    locale === "ar" ? "احصل على عرض أسعار لوجستي | ماكسلاين جلوبال" : "Get a Logistics Quote | Maxline Global";
          ...
          {features.map((item) => (
            ...
            >
              <div className="flex size-16 items-center justify-center rounded-full bg-muted md:size-20">
                <item.icon aria-hidden="true" className="size-10 stroke-[1.5] text-brand-gray md:size-12" />
              </div>
              <h3 className="mt-8 mb-3 font-grotesk text-2xl text-brand-dark md:mt-12 md:text-4xl">{item.title}</h3>
