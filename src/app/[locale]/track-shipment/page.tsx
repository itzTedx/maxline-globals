...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale }> }): Promise<Metadata> {
  ...
  const title = locale === "ar" ? "تتبع شحنتك | ماكسلاين جلوبال" : "Track Your Shipment | Maxline Global";
...
export default async function TrackingRedirect({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params;
    ...
    >
      <div className="py-10 md:py-16">
        <h1 className="mx-auto max-w-4xl text-center font-grotesk text-4xl text-brand-dark tracking-tight sm:text-5xl md:text-6xl lg:text-8xl">
          <StaggeredText duration={0.3} staggerChildren={0.01} text={t("title")} />
        </h1>
        <p className="mx-auto mt-4 max-w-5xl text-center text-lg text-muted-foreground tracking-tight">
          <StaggeredText duration={0.3} staggerChildren={0.01} text={t("description")} />
        </p>
      ...
      <div className="mx-auto mt-4 mb-6 max-w-5xl text-center text-lg tracking-tight">
        <p>
          <StaggeredText delay={0.5} duration={0.3} staggerChildren={0.01} text={t("assistance")} />
        </p>
      </div>
      <Cta />
    </main>
